...
global templatedir 
templatedir = r"D:\Red_Oak_Project\templates\multipagetemplates"
global basemaster
basemaster=r"D:\Red_Oak_Project\master\RedOak Alignment Master_Closed_Polygons.dwg"


def platmultipage(cen,tractnumber,direction,pagelength,pages,scalevalue):
    ...
    parameters['SCALE']=str(scalevalue)
    parameters['DWGTEMPLATE']=pf.returnTemplateName(templatedir,scalevalue)
    ...
    try:
        ...
        return "No Geometry Created For Plat"
    try:
        del wrkspcrunner
    except:
    ...
    for prt in (range(tractcenterlines.partCount)):
        ...
        if len(endmons)<1:
            ...
            endmons = pf.getROWPolyShape(lastbuff,monument,cenname,type="monument",where=wheremon)
        if len(endmons)<1:
            endmons = endmons = pf.getROWPolyShape(lastlinebuff,monument,cenname,type="monument",where=wheremon)
        if len(endmons)<1:
            endmons = endmons = pf.getROWPolyShape(lastlinebuff,monument,cenname,type="monument",where=None)
        if len(endmons)>0:
    ...
    for x in range(pages):
        ...
        distalongline = tractcenterlines.segmentAlongLine(dist1,dist2,False)
        tileindexrow,tileindexfields = pf.createIndexPoly(newgdb,tractnumber,scale,distalongline.centroid.X,distalongline.centroid.Y,sr,tileid=x+1)
    ##########################################################################################################################
    ##
    ## get index polys loop through and dim for each tile
    ##
    ############################################################################################################################
    tiles = pf.getTileIndexShapes(newgdb,"TILE_INDEX")
    ...
    if len(rowintersections)>0:
        ...
        if len(twsintersections)>0:
            insertlist,char = pf.createROWDim(tractcenterlines,newgdb,dimppoint,viewportpointlayer,twss=twsintersections,row=rowtotalshape,tileindex=tiles)
        ...
        if len(twsintersections)<1:
            insertlist,char = pf.createROWDim(tractcenterlines,newgdb,dimppoint,viewportpointlayer)
            listthrough,dictin = pf.createDimDicts(atwsintersections)
            if len(dictin)>0:
                pf.createAtwsViewports(newgdb,viewportpointlayer,dictin,listthrough,char)
